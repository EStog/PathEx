...
In this case ``C`` is a :mod:`short alias <pathpy.expressions.aliases>` of :class:`~.Concatenation`, and the overloaded operator :meth:`* <pathpy.expressions.expression.Expression.__mul__>` with operand :data:`... <Ellipsis>` is used to construct an unbounded repetition. Because the generated language is infinite, helper function :func:`~.take` has being used to select specific amounts of words. The words are taken from the generator returned by :meth:`~.get_eager_generator`.
